...
static struct rkvdec_core *acquire_core(struct rkvdec_dev *rkvdec,
...
void rkvdec_schedule_watchdog(struct rkvdec_core *core, u32 timeout_threshold)
{
	...
	schedule_delayed_work(&core->watchdog_work,
...
static void rkvdec_show_fdinfo(struct seq_file *m, struct file *file)
{
	...
	for (i = 0; i < rkvdec->core_count; i++)
		v4l2_metrics_show_clock(m, rkvdec->cores[i].axi_clk, i);
}
