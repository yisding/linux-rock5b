	...
	if (method == TXAGC) {
		...
		if (*dm->mp_mode == true) {
			if (rf_path == RF_PATH_A) {
				...
				phy_set_bb_reg(adapter, REG_TX_AGC_B_CCK_11_A_CCK_2_11, 0x00ffffff, tx_agc);
				RF_DBG(dm, DBG_RF_TX_PWR_TRACK,
				       "%s: CCK Tx-rf(A) Power = 0x%x\n",
				       __func__, tx_agc);
				...
				RF_DBG(dm, DBG_RF_TX_PWR_TRACK,
				       "%s: OFDM Tx-rf(A) Power = 0x%x\n",
				       __func__, tx_agc);
			} else if (rf_path == RF_PATH_B) {
				/* CCK path B */
				pwr = phy_query_bb_reg(adapter, REG_TX_AGC_B_RATE18_06, 0xFF);
				pwr += dm->rf_calibrate_info.power_index_offset[RF_PATH_B];
				phy_set_bb_reg(adapter, REG_TX_AGC_B_CCK_1_55_MCS32, MASKBYTE3, pwr);
				phy_set_bb_reg(adapter, REG_TX_AGC_B_CCK_11_A_CCK_2_11, 0xff000000, pwr);
				RF_DBG(dm, DBG_RF_TX_PWR_TRACK,
				       "%s: CCK Tx-rf(B) Power = 0x%x\n",
				       __func__, pwr);
				...
				tx_agc = ((pwr << 24) | (pwr << 16) | (pwr << 8) | pwr);
				phy_set_bb_reg(adapter, REG_TX_AGC_B_RATE18_06, MASKDWORD, tx_agc);
				phy_set_bb_reg(adapter, REG_TX_AGC_B_RATE54_24, MASKDWORD, tx_agc);
				phy_set_bb_reg(adapter, REG_TX_AGC_B_MCS03_MCS00, MASKDWORD, tx_agc);
				phy_set_bb_reg(adapter, REG_TX_AGC_B_MCS07_MCS04, MASKDWORD, tx_agc);
				phy_set_bb_reg(adapter, REG_TX_AGC_B_MCS11_MCS08, MASKDWORD, tx_agc);
				phy_set_bb_reg(adapter, REG_TX_AGC_B_MCS15_MCS12, MASKDWORD, tx_agc);
				RF_DBG(dm, DBG_RF_TX_PWR_TRACK,
				       "%s: OFDM Tx-rf(B) Power = 0x%x\n",
				       __func__, tx_agc);
			}
		} else
#endif
		{
			...
			if (rf_path == RF_PATH_A) {
				...
				phy_set_tx_power_index_by_rate_section(adapter, RF_PATH_A, *dm->channel, HT_MCS8_MCS15);
			} else {
				phy_set_tx_power_index_by_rate_section(adapter, RF_PATH_B, *dm->channel, CCK);
				phy_set_tx_power_index_by_rate_section(adapter, RF_PATH_B, *dm->channel, OFDM);
				phy_set_tx_power_index_by_rate_section(adapter, RF_PATH_B, *dm->channel, HT_MCS0_MCS7);
				phy_set_tx_power_index_by_rate_section(adapter, RF_PATH_B, *dm->channel, HT_MCS8_MCS15);
			}
		}
