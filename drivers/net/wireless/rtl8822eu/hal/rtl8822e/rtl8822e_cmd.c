 ...
 *
 ******************************************/
s32 rtl8822e_fillh2ccmd(PADAPTER adapter, u8 id, u32 buf_len, u8 *pbuf)
{
...
void _rtl8822e_set_FwPwrMode_cmd(PADAPTER adapter, u8 psmode, u8 rfon_ctrl)
{
	int i;
...
void rtl8822e_set_FwPwrMode_cmd(PADAPTER adapter, u8 psmode)
{
	return _rtl8822e_set_FwPwrMode_cmd(adapter, psmode, rf_off);
}

void rtl8822e_set_FwPwrMode_rfon_ctrl_cmd(PADAPTER adapter, u8 rfon_ctrl)
{
	struct pwrctrl_priv *pwrpriv = adapter_to_pwrctl(adapter);

	return _rtl8822e_set_FwPwrMode_cmd(adapter, pwrpriv->power_mgnt, rfon_ctrl);
}

void rtl8822e_set_FwPwrModeInIPS_cmd(PADAPTER adapter, u8 cmd_param)
{
...
void rtl8822e_set_fw_pwrmode_inips_cmd_wowlan(PADAPTER padapter, u8 ps_mode)
{
	...
	rtl8822e_fillh2ccmd(padapter, H2C_INACTIVE_PS_, sizeof(param), param);
}

#endif /* CONFIG_WOWLAN */

void rtl8822e_set_usb_suspend_mode(PADAPTER padapter)
{
	struct pwrctrl_priv *ppwrpriv = adapter_to_pwrctl(padapter);
	u8 param[H2C_BT_UNKNOWN_DEVICE_WA_LEN] = {0};

	SET_H2CCMD_BT_UNKNOWN_DEVICE_WA_FORCE_IB_EN(param, 1);
	rtl8822e_fillh2ccmd(padapter, H2C_BT_UNKNOWN_DEVICE_WA, H2C_BT_UNKNOWN_DEVICE_WA_LEN, param);
}
...
#ifdef CONFIG_BT_COEXIST
void rtl8822e_download_BTCoex_AP_mode_rsvd_page(PADAPTER adapter)
{
...
static void process_c2h_event(PADAPTER adapter, u8 *c2h, u32 size)
{
	...
	desc_size = rtl8822e_get_rx_desc_size(adapter);
	...
	case CMD_ID_C2H_AP_REQ_TXRPT:
		...
		C2HTxRPTHandler_8822e(adapter, pc2h_data, c2h_len);
		break;

	case CMD_ID_C2H_SPECIAL_STATISTICS:
		/*RTW_INFO("[C2H], C2H_SPC_STAT!!\n");*/
		C2HSPC_STAT_8822e(adapter, pc2h_data, c2h_len);
		break;
...
void rtl8822e_c2h_handler(PADAPTER adapter, u8 *pbuf, u16 length)
{
#ifdef CONFIG_WOWLAN
	struct pwrctrl_priv *pwrpriv = adapter_to_pwrctl(adapter);
 ...
 */
void rtl8822e_c2h_handler_no_io(PADAPTER adapter, u8 *pbuf, u16 length)
{
	u32 desc_size;
	u8 id, seq;
	u8 *pc2h_content;
