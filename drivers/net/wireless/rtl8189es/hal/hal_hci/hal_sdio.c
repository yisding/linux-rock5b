...
static void dump_mac_page0(PADAPTER padapter)
{
	...
	for (index = 0 ; index < 0x100 ; index += 16) {
		...
		strscpy_pad(str_out, str_val, len);
#else
...
#if defined(CONFIG_RTL8188F) || defined(CONFIG_RTL8188GTV) ||defined(CONFIG_RTL8188E) || defined(CONFIG_RTL8821A)
