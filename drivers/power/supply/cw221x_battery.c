...
#define CW_PROPERTIES "bat-cw221X"
...
static int cw221x_parse_properties(struct cw_battery *cw_bat)
{
	...
	if (length < 0) {
		dev_warn(cw_bat->dev,
			 "No battery-profile found, using the built-in profile\n");
		return length;
	}
...
static int cw221X_probe(struct i2c_client *client)
{
	...
	if (!cw_bat->cwfg_workqueue) {
		dev_err(cw_bat->dev, "Failed to create gauge workqueue\n");
		return -ENOMEM;
	}
