...
* RGA3 Porter-Duff alpha blend for the common A+B composition paths emitted by
  ...
  conversions, and the default ffmpeg/RKMPP overlay path where a semiplanar
  8/10-bit YUV main/foreground image and RGB/RGBA pattern write a YUV
  destination, including mixed-depth source/writeback and AFBC16x16 output when
  requested, through the per-window CSC and compact-10-bit policy generated by
  ``librga``.  Main-request rotation/mirror is applied to the
