...
* RGA3 Porter-Duff alpha blend for the common A+B composition paths emitted by
  ...
  ``librga``.  Rotation/mirror with alpha, 10-bit YUV-destination alpha,
  no-pattern YUV A+B->B, and compressed in-place write-back remain unsupported.
...
* RGA2 raster bitblit for the common upstream-consumer fallback formats that
  ...
  YUV444 semiplanar, RGB555-family, and ARGB/ABGR output.  This path supports
  imported dma-buf backed source/destination images, single-handle and
  per-plane-handle images, linear raster scaling/conversion, BSP-compatible
  ``rga_req.rotate_mode`` plus ``sina``/``cosa`` rotation and mirror encoding,
  including the active-rectangle swap used by ``librga`` for 90/270-degree
  rotation, and job-owned dma-buf mappings for the selected RGA2 core.  RGA2
  destination full-CSC coefficients generated by ``librga`` for RGB-to-YUV
  conversion are accepted for this raster bitblit path and programmed through
  the BSP-compatible CSC register block.  Pattern, color-key, alpha/ROP,
  tile/FBC, 10-bit RGA2-only input, OSD, mosaic, pre-intr, and gauss variants
  remain unsupported.
...
* Optional ``ROCKCHIP_RGA_REWRITE_KUNIT_TEST`` coverage for rewrite-local ABI
  ...
  dispatch, and ffmpeg-facing RGA3 raster/FBC/alpha-overlay profile selection.
...
* RGA3 pattern outside the supported alpha-overlay profile, color-key, alpha
  plus rotation/mirror, no-pattern or 10-bit YUV-destination alpha,
  per-channel rotation, RFBC/tile/AFBC32x8, 10-bit or AFBC destination offsets,
