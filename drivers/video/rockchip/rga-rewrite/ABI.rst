...
* RGA3 Porter-Duff alpha blend for the common A+B composition paths emitted by
  ...
  overlay path where an 8-bit or matching semiplanar 10-bit YUV
  main/foreground image and RGB/RGBA pattern write a YUV destination through
  the per-window CSC policy generated by ``librga``.  Main-request
  ...
  YUV-destination alpha, no-pattern YUV A+B->B, and compressed in-place
  write-back remain unsupported.
