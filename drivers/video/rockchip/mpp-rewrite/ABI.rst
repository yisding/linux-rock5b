...
* RK3588 VDPU383/RKVDEC2 hard-CCU link-table capability data, backing
  ...
  decoder jobs stage the BSP write/readback register partitions into the first
  table node so the table-image format is validated before hard-CCU dispatch is
  enabled.  The readback helper copies BSP table readback partitions back into
  the normal register image and applies the VDPU383 link-mode status word.
...
* Optional ``ROCKCHIP_MPP_REWRITE_KUNIT_TEST`` coverage for rewrite-local ABI
  ...
  checks, BSP VDPU383 link-table layout/materialization/readback,
...
* Full BSP-equivalent RK3588 decoder/link CCU task policy and SRAM-backed
  ...
  preparation, and VDPU383 link-table materialization/readback helpers, but
  does not yet mirror the BSP decoder/link scheduling and hard-CCU table
  submission policy.
