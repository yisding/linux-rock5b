...
* Public IOMMU fault callback registration for bound MPP cores.  Rockchip-IOMMU
  ...
  removal clears the matching callback with the provider's atomic-safe
  setter and then drains any in-flight provider IRQ callback with the
  matching sleepable sync before releasing the hardware object.  A fault
